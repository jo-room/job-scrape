...
def get_new_relevant_jobs(
    ...
    existing_jobs = defaultdict(
        set, {key: set(value) for key, value in run_record.existing_jobs.items()}
    )
    ...
    for company, job in relevant_jobs:
        if job.id not in existing_jobs[company.name]:
            if company.name in new_relevant_jobs:
                ...
                new_relevant_jobs[company.name] = {"company": company, "jobs": [job]}
    ...
    if len(errors) > 0:
        ...
        for company, error in errors:
            ...
            messages.append(
                f"{"NEW ERROR " if is_new_this_run else ""}{company.name}{f" ({company.name}, {company.jobs_page})" if {company.name} else ""}: {getattr(error, 'msg', repr(error))}\n{''.join(traceback.format_exception(error))}"
            )
        ...
        if company.active:
            ...
            try:
                if company.is_crunchbase:
                    company_relevant_jobs, jobs_page_status = get_crunchbase_companies(
                        driver, company, default_sleep
                    )
                else:
                ...
                elif jobs_page_status in {
                    JobsPageStatus.GENERIC_NO_JOBS_PHRASE_FOUND,
                    JobsPageStatus.NO_JOBS_PHRASE_NOT_FOUND_BUT_NO_JOBS,
                }:
    ...
    if company_has_jobs:
        ...
        if company.jobs_page_class:
            ...
            relevant_jobs = [
                job
                for job in jobs
                if title_is_relevant(company, job.title, search_terms)
            ]
        else:
            raise Exception(
                f"Scrape not implemented. Body text: {driver.find_element(By.TAG_NAME, 'body').text.replace("\n", " ")}"
            )
    else:
...
def format_errors_message(errors: list[ScrapeError]) -> str:
    if len(errors) > 0:
        return "Errors:\n" + "\n".join(
            [
                f"{"NEW ERROR " if error.is_new_this_run else ""}{error.company_name}{f" ({error.company_name}, {error.jobs_page})" if {error.company_name} else ""}: {error.message}"
                for error in errors
            ]
        )
    return "No errors."
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "config_file", type=str, help="Path to config.json or config.py file"
    )
    parser.add_argument(
        "run_record_json", type=str, help="Path to file storing history of prior run(s)"
    )
    parser.add_argument(
        "--add_scrapers_file",
        type=str,
        default=None,
        help="Path to file with additional scrapers",
    )
    parser.add_argument(
        "--limit_company",
        type=str,
        default=None,
        help="Search only companies that contain this string in their name",
    )
    parser.add_argument(
        "--add_search_term",
        type=str,
        default=None,
        help="Search term to add in considering a job relevant",
    )
    parser.add_argument(
        "--dont_write_run_record",
        action="store_true",
        help="Don't write the run record file",
    )
    parser.add_argument(
        "--dont_replace_run_record",
        action="store_true",
        help="Don't replace the run record file",
    )
    parser.add_argument(
        "--backup_run_record",
        action="store_true",
        help="Rename the original run record file to <original file name>_backup.json",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Seconds to sleep on load and after scroll",
    )
    ...
    if args.add_scrapers_file:
        additional_scrapers_module = import_from_path(
            "scrapers", args.add_scrapers_file
        )
    else:
    ...
    if len(verify_no_jobs) > 0:
        print(
            bcolors.OKBLUE
            + "No jobs at all, but no specific no jobs phrase:\n"
            + "\n".join([company.name for company in verify_no_jobs])
            + bcolors.ENDC
        )
    if len(new_relevant_jobs) > 0:
        print(
            bcolors.OKGREEN + format_new_jobs_message(new_relevant_jobs) + bcolors.ENDC
        )
