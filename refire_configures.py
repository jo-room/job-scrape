...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Re-create scheduleConfig.json objects to thus re-trigger configuration lambda')
    parser.add_argument('--usernames', type=str, required=True, help="Comma-delimited string to usernames to re-trigger configuration for")
    ...
    for username in usernames:
        ...
        try:
            ...
            if e.response['Error']['Code'] == "404":
                print(bcolors.OKBLUE + f"{username} schedule config not found" + bcolors.ENDC)
                continue
            else:
              raise e

        succeeded = update_schedule_from_config(args.bucket_name, key, should_email=False)
        if succeeded:
