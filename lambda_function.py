...
def lambda_handler(event, context, local=False):
    ...
    dont_replace_existing = (
        event["dont_replace_existing"] if "dont_replace_existing" in event else False
    )
    dont_write_existing = (
        event["dont_write_existing"] if "dont_write_existing" in event else False
    )
    ...
    assumed_role_object = sts.assume_role(
        ...
        Policy=get_inline_session_policy(
            event["aws_config"]["bucket_name"],
            username,
            event["aws_config"]["sns_topic_arn"],
        ),
    ...
    with tempfile.TemporaryDirectory() as tmp_config_scrapers_folder:
        ...
        if "config_json" in event["aws_config"]:
            config_object = s3.Object(
                event["aws_config"]["bucket_name"], event["aws_config"]["config_json"]
            )
    ...
    if dont_replace_existing:
        ...
        run_record_object = s3.Object(
            event["aws_config"]["bucket_name"],
            f"{path}_{str(datetime.datetime.now()).replace(" ", "_")}{extension}",
        )
        ...
        "Statement": [
            {
                ...
                "Condition": {"StringLike": {"s3:prefix": [f"{username}/*"]}},
            },
            {
                "Action": ["s3:GetObject", "s3:PutObject"],
                "Effect": "Allow",
                "Resource": [f"arn:aws:s3:::{bucket_name}/{username}/*"],
            },
            {
                "Action": "sns:Publish",
                "Effect": "Allow",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run with aws resources to scrape new jobs and notify."
    )
    parser.add_argument(
        "event_json",
        type=str,
        help="""Path to lambda event json. Expected structure:
{
    "aws_config": {
        "bucket_name": "",
        "config_file": "",
    ...
    with open(args.event_json) as f:
        ...
        lambda_handler(lambda_event, context=None, local=True)
